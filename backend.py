...
- 사용자가 일반 지식, 음식, 공부, 설명, 정리 요청을 하면 먼저 기본 답변을 제공하세요. 추가 정보가 필요하더라도 질문만 하고 끝내지 마세요. 기본 답변을 2~5문장으로 짧게 제시한 뒤, 필요한 경우 마지막에 선택 질문을 덧붙이세요.
...
def build_user_prompt(payload):
    ...
    last_free_topic = payload.get('lastFreeTopic')
    if last_free_topic:
        parts.append("\n[최근 자유 대화 주제]")
        parts.append(f"사용자 요청: {last_free_topic.get('userText', '')}")
        parts.append(f"이전 답변: {last_free_topic.get('botText', '')}")
        parts.append("이번 입력은 이 주제를 이어가거나, 이전 답변이 부족하다고 느껴 다시 요구하는 상황입니다. 같은 질문을 다시 묻지 말고, 이전 주제를 더 구체적으로 이어서 답하세요.")
        if payload.get('intent') == 'frustration':
            parts.append("사용자가 답변이 답답하다고 느끼고 있습니다. 방어적으로 사과만 하지 말고, 짧게 수습한 뒤 바로 이전 주제에 대해 더 도움이 되는 답변을 제공하세요.")

    # 이번 사용자 메시지
    parts.append(f"\n[이번 사용자 메시지]\n사용자: {user_msg}")
    parts.append("\n먼저 이번 사용자 메시지에 직접 답하세요. 사용자가 일반 지식, 음식, 공부, 설명, 정리 요청을 하면 질문만 하지 말고 기본 답변을 2~5문장으로 짧게 먼저 제시하세요. 필요할 때만 위 맥락을 톤 조절에 참고하세요. 사용자가 상담을 원하지 않는 일반 질문, 음식, 공부, 농담, 기능 테스트, 잡담을 말하면 일반 대화처럼 답하세요. 다른 설명·번호·접두어 없이 답변만:")
